...
function openChildModal(parentId, parentName) {
    ...
    const parentNameSpan = document.getElementById('parent-name-display');
    const parentIdInput = document.getElementById('modal-parent-id');

    if (parentIdInput) parentIdInput.value = parentId || '';
    if (parentNameSpan) parentNameSpan.textContent = parentName || '';
    if (modal) {
...
function openInfoModal(contentId) {
    ...
    const node = nodeMap[contentId];
    if (!modal || !node) return;

    const nameEl = document.getElementById('info-node-name');
    const idEl = document.getElementById('info-node-id');
    const statusEl = document.getElementById('info-node-status');
    const descEl = document.getElementById('info-node-description');
    const pathEl = document.getElementById('info-path');

    if (nameEl) nameEl.textContent = node.name || '';
    if (idEl) idEl.textContent = node.contentId || '';
    if (statusEl) {
        statusEl.textContent = node.status || 'New';
        const classes = getStatusClasses(node.status);
        statusEl.className = `px-2 py-0.5 rounded text-xs font-medium ${classes.badge}`;
    }
    if (descEl) descEl.innerHTML = linkifyDescription(node.description || '');
    if (pathEl) pathEl.textContent = formatTreePath(getBreadcrumbPath(contentId));
...
function openEditModal(contentId) {
    ...
    document.getElementById('edit-status').value = node.status || 'New';
...
function openDeleteConfirmModal(contentId) {
    ...
    document.getElementById('confirm-delete-button').onclick = () => deleteNode(contentId, node.name || '');
    const nameSpan = document.getElementById('delete-node-name');
...
function openSearchLinkModal(parentId, parentName) {
    const modal = document.getElementById('search-link-modal');
    if (!modal) return;
